...
- `test_suite_without_new` is re-run on a checkout of `base` whenever it is
  ...
  `ENVIRONMENT: ` problem, not as pre-existing breakage. That base run reads
  base's own test files. The branch's are not copied onto base for that run,
  so your selector alone keeps the ticket's nodes out of it, and that
  selector must be a no-op when it names a test base does not have yet
  (pytest's `--deselect` is; a runner that errors on an unknown selector
  must be wrapped, like the selector trap above).
