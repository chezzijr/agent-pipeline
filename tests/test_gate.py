...
def test_gate_skips_a_test_the_plan_declares_deleted():
    """TICKET-138: Tier A must not execute a reproduction the plan deletes.

    A deleted test is not suite pre-existing breakage. The declaration is data,
    rather than a phrase in `## Plan`, because prose cannot distinguish deleting
    a file from deleting one assertion inside it.
    """
    d = project(FIXTURE.replace(
        "test_file: test_thing.py::test_broken",
